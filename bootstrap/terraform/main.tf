...
resource "aws_iam_openid_connect_provider" "default" {
  ...
  thumbprint_list = ["63966130761608209718C5045CFFB4856FB53976"]
}

resource "aws_s3_bucket" "sandbox" {
  bucket = "satish-irsa-podidentity1"
  provider = aws.sandbox
}
    ...
    Statement = [
      {
        ...
        "Condition": {
                "StringEquals": {
                    ...
                    format("%s:sub",module.eks.oidc_provider): "system:serviceaccount:default:s3-access-app",                    
                }
            }
      }        
...
module "eks_blueprints_addons" {
  ...
  #Satish disable  grafana due to security finding
...
resource "kubectl_manifest" "environmentconfig" {
  yaml_body = templatefile("${path.module}/config/environmentconfig.yaml", {
    ...
    roleArn      = aws_iam_role.satish-irsa-podidentity.arn
    bucketName   = aws_iam_role.satish-irsa-podidentity.id
  })
